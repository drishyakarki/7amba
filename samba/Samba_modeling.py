...
import torch
import math
from samba.Samba_config import SambaConfig
import logging
...
from utils import ModelOutput, ACT2FN

class MambaCache:
    def __init__(
        self, config: SambaConfig, batch_size: int, dtype: torch.dtype = torch.float16, device: Optional[str] = None
    ):
...
class SambaAttentionDecoder(nn.Module):
    def __init__(self, config: SambaConfig):
        ...
        self.pre_ff_layernorm = nn.LayerNorm(config.hidden_size)
...
class SambaModel(MambaPreTrainedModel):
    def __init__(self, config):
        ...
        self.sanity_check_layer_alternation(config)

    def sanity_check_layer_alternation(self, config):
        for idx, layer in enumerate(self.layers):
            if (idx + 1) % config.attention_layer == 0:
                if not isinstance(layer, SambaAttentionDecoder):
                    raise ValueError(f"Expected SambaAttentionDecoder at index {idx}, but found {type(layer)}")
            else:
                if not isinstance(layer, MambaBlock):
                    raise ValueError(f"Expected MambaBlock at index {idx}, but found {type(layer)}")
        
        print("Sanity check passed: Layer alternation is correct.")
